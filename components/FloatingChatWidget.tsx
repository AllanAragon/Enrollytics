...
export default function FloatingChatWidget() {
  ...
  return (
    <>
      ...
      {/* Chat Panel */}
      {isOpen && (
        <div className="fixed bottom-6 right-6 w-[420px] h-[650px] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col z-50 animate-in slide-in-from-bottom-4 duration-300">
          {/* Header */}
          <div className="flex items-center gap-3 p-4 border-b border-gray-200 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-t-2xl">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
              ...
              <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                    msg.role === 'user' ? 'bg-indigo-600' : 'bg-white border-2 border-purple-200'
                  }`}
                >
                  {msg.role === 'user' ? (
            ...
            {loading && (
              <div className="flex gap-3">
                ...
                <div className="bg-white border border-gray-100 rounded-2xl rounded-tl-md px-4 py-3 flex items-center gap-1.5 shadow-sm">
          ...
          <div className="p-4 border-t border-gray-200 bg-white rounded-b-2xl">
            <div className="flex items-end gap-2">
              <textarea
                ...
                placeholder="Ask about enrollment insights…"
                rows={2}
                maxLength={MAX_MESSAGE_LENGTH}
                className="flex-1 resize-none rounded-xl border border-gray-300 px-3 py-2 text-sm text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent placeholder-gray-400"
                disabled={loading}
              />
              <button
                onClick={handleSend}
                disabled={!input.trim() || loading}
                className="flex-shrink-0 w-10 h-10 rounded-xl bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white flex items-center justify-center transition-colors shadow-sm"
              >
